...
def main() -> None:
    ...
    ap.add_argument("--nonsym", "--nonsymmetric", action="store_true", dest="nonsym",
    ...
    score_maps = {length: c.lexicon.load(args.list, length).score_map
                  for length in range(3, args.max_len + 1)}
    ...
    for seed in range(args.seeds):
        if args.gibbs:
            found = gibbs_layout.fill_gibbs(
                args.rows, args.cols, mlex, rng_factory=c.rng_factory, max_len=args.max_len,
                seed=seed, min_len=3, symmetric=symmetric, distinct=True,
                black_fraction=args.black_fraction, target_black=None, max_layouts=60,
            )
        else:
            found = patterns.fill_capped(
                args.rows, args.cols, mlex, rng_factory=c.rng_factory, max_len=args.max_len,
                seed=seed, min_len=3, symmetric=symmetric, distinct=True,
                max_black=args.max_black, layout_node_budget=args.budget,
            )
        if found is None:
        ...
        print(f"  seed {seed:>2}: weakest {weak_word.upper()}({weak_score:.0f})  "
