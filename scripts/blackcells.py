...
def fill_demo(name, template, min_score=50.0, seed=0):
    ...
    mlex = MultiLexicon.from_scored_files(
        lambda n: DATA / f"cw_{n}.txt", g.lengths_needed(), min_score=min_score
    )
...
def ground_truth_check():
    ...
    mlex = MultiLexicon.from_scored_files(
        lambda n: DATA / f"scored_{n}.txt", g.lengths_needed(), min_score=3.5
    )
...
def unsat_demo():
    ...
    for bar in (50, 70, 85, 90, 92):
        mlex = MultiLexicon.from_scored_files(
            lambda n: DATA / f"cw_{n}.txt", g.lengths_needed(), min_score=bar
        )
        sizes = {n: len(mlex.get(n)) for n in sorted(g.lengths_needed())}
        t0 = time.perf_counter()
