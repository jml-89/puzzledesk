...
:root{
  ...
  /* text roles — bright, high-contrast on bg; these are the only colours letters use */
  --ink:#e7edf3;--muted:#8ba0b4;--faint:#54677a;--given:#f0d69b;--logic:#5fd0d6;
  /* structural (non-text) roles — dim, well below the text luminance, so proximal/overlapping
     letters always read in front of them. Never used for text. */
  --struct:#357a80;--struct-dim:rgba(95,208,214,.14);--struct-gold:#8c7748;
  --lit-wash:rgba(95,208,214,.085);
...
.cell.lit{background:var(--lit-wash)}
.cell.lit input{color:var(--logic)}
.cell.donor{outline:2px solid var(--struct-gold);outline-offset:-2px}
.cell.pop{animation:pop .55s ease}
@keyframes pop{0%{background:var(--logic-soft)}40%{background:rgba(95,208,214,.30);box-shadow:inset 0 0 0 2px var(--struct)}100%{background:var(--lit-wash)}}
.thread{position:absolute;inset:0;z-index:3;width:100%;height:100%;pointer-events:none;overflow:visible}
.thread .edge{fill:none;stroke:var(--struct-dim);stroke-width:.02;stroke-dasharray:.09 .07;stroke-linecap:round;transition:stroke .3s,stroke-width .3s}
.thread .edge.live{stroke:var(--struct);stroke-width:.04;stroke-dasharray:none}
.thread .edge.hot{stroke:var(--struct-gold);stroke-width:.045;stroke-dasharray:none}
.thread .node{fill:var(--cell);stroke:var(--struct-dim);stroke-width:.03;transition:fill .3s,stroke .3s}
.thread .node.live{fill:var(--struct);stroke:var(--struct)}
...
.clist .wv{display:inline-block;font-family:var(--mono);font-size:10px;color:var(--logic);border:1px solid var(--logic-dim);border-radius:3px;padding:0 4px;margin-right:6px;vertical-align:1px}
.clist li.done .wv{color:var(--faint);border-color:var(--edge)}
...
<div class="wrap">
  ...
  <h1>Latent</h1>
  <p class="sub">Every crossword is two puzzles at once: a <b>trivia</b> puzzle and a <span class="lg">logic</span> puzzle the crossings hide. Here you get only <b>four</b> clues. The other <span class="lg">six</span> answers are never given — you deduce each from the letters its crossings force. Watch each blank light up as its crossings pin it.</p>
  <div class="rule"></div>
  ...
  <div class="play">
    ...
    <div class="side">
      ...
      <div class="status" id="status">Four clues in gold. The six in cyan have none — solve the gold, and its letters (ringed) force each blank. Numbers are deduction waves; pick a cyan clue to see which letters pin it.</div>
    </div>
  </div>
...
(function(){
  ...
  function render(){
    for(var r=0;r<R;r++)for(var c=0;c<C;c++){var cl=document.getElementById("cell-"+r+"-"+c);cl.classList.remove("inline","active","donor");}
    if(typeof edgesG!=="undefined")edgesG.querySelectorAll(".edge.hot").forEach(function(ln){ln.classList.remove("hot");});
    var e=cur();
    if(e)e.cells.forEach(function(x){document.getElementById("cell-"+x[0]+"-"+x[1]).classList.add("inline");});
    if(e&&e.role!=="given"){(e._donors||[]).forEach(function(x){cellEl(x).classList.add("donor");});edgesFor(e.label).forEach(function(ln){ln.classList.add("hot");});}
  ...
  function paint(e){var li=document.getElementById("li-"+e.dir+e.num);var ct=li.querySelector(".ct");
    ...
    else ct.innerHTML='<span class="wv" title="deduction wave">'+e.wave+'</span>'+hint(e)+' <span style="color:var(--faint)">('+e.len+')</span>';
    li.classList.toggle("done",isDone(e));li.classList.toggle("on",cur()===e);}

  // the forcing graph: a deduced entry is pinned by the crossing letters its neighbours donate
  // in an earlier wave. Draw an edge from every such donor cell to the entry, anchored on the
  // real letter that forces it, and light them -- wave by wave -- as the crossings solve each word.
  function svg(t){return document.createElementNS("http://www.w3.org/2000/svg",t);}
  function cellEl(x){return document.getElementById("cell-"+x[0]+"-"+x[1]);}
  function centroid(e){var sx=0,sy=0;e.cells.forEach(function(x){sx+=x[1]+0.5;sy+=x[0]+0.5;});return [sx/e.cells.length,sy/e.cells.length];}
  function crossing(e,x){return (e.dir==="A"?mapD:mapA)[key(x[0],x[1])];} // entry crossing e at cell x
  function donors(e){ // e's own cells whose crossing entry solves earlier -> letters known before e's wave
    if(e.role==="given")return [];
    return e.cells.filter(function(x){var o=crossing(e,x);return o&&o.wave<e.wave;});
  }
  var OFF=0.30; // nudge each run off the letter centres (a blank's donors lie in its own line) so glyphs stay legible
  var deduced=all.filter(function(e){return e.role!=="given";});
  var edgesG=document.getElementById("edges"),nodesG=document.getElementById("nodes"),nodeOf={};
  deduced.forEach(function(e){
    var vert=(e.dir==="D"),cn=centroid(e);e._donors=donors(e);
    var nx=cn[0]+(vert?OFF:0),ny=cn[1]+(vert?0:OFF); // node, sitting to the side of the run
    e._donors.forEach(function(x){var ln=svg("line");
      ln.setAttribute("x1",x[1]+0.5+(vert?OFF:0));ln.setAttribute("y1",x[0]+0.5+(vert?0:OFF));
      ln.setAttribute("x2",nx);ln.setAttribute("y2",ny);
      ln.setAttribute("class","edge");ln.setAttribute("data-e",e.label);edgesG.appendChild(ln);});
    var nd=svg("circle");nd.setAttribute("cx",nx);nd.setAttribute("cy",ny);nd.setAttribute("r",".066");
    nd.setAttribute("class","node");nd.id="node-"+e.label;nodesG.appendChild(nd);nodeOf[e.label]=nd;
  });
  function edgesFor(lab){return edgesG.querySelectorAll('[data-e="'+lab+'"]');}
  var litSet={};
  function refreshThread(){
    var nowLit={}; // recompute from scratch so a cell shared by two blanks never gets stomped
    deduced.forEach(function(e){
      var done=isDone(e),lab=e.label;
      nodeOf[lab].classList.toggle("live",done);
      edgesFor(lab).forEach(function(ln){ln.classList.toggle("live",done);});
      if(done){
        e.cells.forEach(function(x){nowLit[key(x[0],x[1])]=true;});
        if(!litSet[lab]){litSet[lab]=true;e.cells.forEach(function(x){var cl=cellEl(x);cl.classList.add("pop");setTimeout(function(){cl.classList.remove("pop");},560);});}
      }else litSet[lab]=false;
    });
    for(var r=0;r<R;r++)for(var c=0;c<C;c++)cellEl([r,c]).classList.toggle("lit",!!nowLit[key(r,c)]);
  }
  ...
  document.getElementById("clearBtn").addEventListener("click",function(){
    ...
    status.textContent="Cleared. Solve the four gold clues; the crossings force the rest.";setActive(0,0,true);refresh();});
